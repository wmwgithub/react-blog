...
import { Icon, Avatar, Input, Popover, Drawer, Button } from "antd";
    ...
    return this.state.showSearch ? (
      <div>
        <Popover
          placement="bottom"
          title="搜索结果"
          content={this.state.content.map(value => (
            <div key={value.id}>
              <a href={`/#/acticle/?userid='userid'&index=${value.id}`}>
                {value.title}
              </a>
              <br />
            </div>
        ...
        >
          <Input
            ...
            suffix={<Icon type="close" onClick={this.showSearch} />}
          />
        </Popover>
      </div>
    ) : (
